...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Элементы.ОтборХозОперация.СписокВыбора.ЗагрузитьЗначения(ЗначениеНастроекПовтИсп.ПолучитьХозОперацииДокумента("ТК_Переработка"));
...
Процедура ПриОткрытии(Отказ)
	...
	КонецЕсли;
	Если ЗначениеЗаполнено(ОтборСкладКомпании) Тогда 
		ПараметрыОтбора.Вставить("СкладКомпании", ОтборСкладКомпании);
	КонецЕсли;
