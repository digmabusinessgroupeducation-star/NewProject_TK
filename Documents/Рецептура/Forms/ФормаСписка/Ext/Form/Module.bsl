...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//СписокХозОпераций = ЗначениеНастроекПовтИсп.ПолучитьСписокХозОпераций("Рецептура");	
	//Элементы.ОтборХозОперация.СписокВыбора.ЗагрузитьЗначения(СписокХозОпераций.ВыгрузитьЗначения());
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ОтборПродукция) Тогда 
		ПараметрыОтбора.Вставить("Продукция", ОтборПродукция);
	КонецЕсли;
...
Процедура ОтборПродукцияПриИзменении(Элемент)
	...
	УстановитьОтборСписка(Список, Новый Структура("Продукция", ОтборПродукция));		
