...
Функция РазобратьШтриховойКодТовара(Знач ШтриховойКодТовара) Экспорт;
	...
	ДанныеКодаТовара.Вставить("Разобран", Ложь);
	...
	ДанныеКодаТовара.Вставить("РевизитКодаТовараHEX"); // Значение реквизита "код товара" , тег 1162 в бинарном виде.
	ДанныеКодаТовара.Вставить("РевизитКодаТовара"); // Значение реквизита "код товара", тег 1162  в BASE64.
	
	// Алгоритм реализован по ПРОЕКТУ приказа ФНС России от 29.08.2019
	...
	Если ДлинаШтрихкода = 20 Тогда // Код товара средства идентификации мехового изделия.
		// Последовательности данных равна 20(21) символам
		// Последовательность символов состоит из прописных латинских букв, цифр и символа-разделителя «-»,
		// проверяется формат содержания считанной последовательности на шаблон СС-ЦЦЦЦЦЦ-ССССССССССС.
		Если Сред(ШтриховойКодТовара, 3, 1) = "-" И Сред(ШтриховойКодТовара, 10, 1) = "-" Тогда
			...
			СерияИдентификационногоЗнака = Сред(ШтриховойКодТовара, 11, 11);
	...
	Если ДлинаШтрихкода = 29 Тогда // Код товара в формате Data Matrix маркировки табачной продукции.
		Если ПроверкаСтроки(ШтриховойКодТовара, Истина, Истина, Истина, "«!”%&’()*+-.,:;=<>?»") Тогда
			...
			Если МенеджерОборудованияКлиентСервер.РассчитатьКонтрольныйСимволGTIN14(GTIN) = Прав(GTIN, 1) Тогда
	...
	Если ДанныеКодаТовара.Разобран Тогда
		...
		Если GTIN <> Неопределено И МенеджерОборудованияКлиентСервер.РассчитатьКонтрольныйСимволGTIN14(GTIN) = Прав(GTIN, 1) Тогда
			ДанныеКодаТовара.ТипИдентификатораТовара = ПредопределенноеЗначение("Перечисление.ТипыИдентификаторовТовараККТ.КодТовараВФорматеDataMatrixGS1");
