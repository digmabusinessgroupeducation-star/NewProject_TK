...
Процедура ПолучитьПланыОбмена(ПланыОбменаПодсистемы) Экспорт
	 ...
	 ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменСУпрДок);
	 ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменConsolidation);
	 ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменБухгалтерия);
	 ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена.ОбменФабрикаСыра);
...
// Получает правила конвертации объектов из ИБ для плана обмена.
//
// Параметры:
//  ИмяПланаОбмена - Строка - имя плана обмена как объекта метаданных.
// 
// Возвращаемое значение:
//  Правила - ХранилищеЗначения - правила конвертации объектов.
//  Неопределено - если правила конвертации не были загружены в базу для плана обмена.
//
Функция ПравилаКонвертацииОбъектов(Знач ИмяПланаОбмена, ПолучатьПравилаКорреспондента = Ложь) Экспорт
	
	// Возвращаемое значение функции.
	ПравилаXML = Неопределено;
	
	ТекстЗапроса = "ВЫБРАТЬ
	|	ПравилаДляОбменаДанными.%1 КАК ПравилаXML
	|ИЗ
	...
	ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстЗапроса,
		?(ПолучатьПравилаКорреспондента, "ПравилаXMLКорреспондента", "ПравилаXML"));
	...
	Если Не Результат.Пустой() Тогда
		...
		СтрокаXML = ПолучитьСтрокуИзДвоичныхДанных(Выборка.ПравилаXML.Получить(),КодировкаТекста.UTF8);
		
		ПравилаXML	= Новый ХранилищеЗначения(СтрокаXML, Новый СжатиеДанных(9));	
