...
Процедура ВключениеТехнологическогоЖурнала(ПараметрыТехнологическогоЖурнала, РезультатВключения) Экспорт
	...
	Если КаталогКонфигурацииПриложения <> Неопределено Тогда
		...
		ПараметрыФайлаКонфигурации.Вставить("КаталогСЛогФайлами", КаталогСЛогФайлами);
		...
		ЗаписатьКонфигурациюВФайл(СтруктураФайлаКонфигурации, ПараметрыФайлаКонфигурации, РезультатВключения);
...
Процедура ВыключениеТехнологическогоЖурнала(КаталогСЛогФайлами) Экспорт
	...
	ЗаписатьКонфигурациюВФайл(СтруктураФайлаКонфигурации, ПараметрыФайлаКонфигурации, РезультатВыключения);
...
Процедура ДобавитьБлокВСтруктуруКонфигурации(СтруктураФайлаКонфигурации, ПараметрыТекста, ЭтоБлокКонсоли)
	...
	БлокКонфигурации.Вставить("КаталогТЖ");
	...
	Если НЕ СтруктураФайлаКонфигурации.ЕстьPlanSql Тогда
		Если СтрНайти(ПараметрыТекста.ТекстБлока, "<plansql/>") > 0 Тогда
...
Функция ЗаписатьКонфигурациюВФайл(СтруктураФайлаКонфигурации, ПараметрыФайлаКонфигурации, РезультатВключения)
	...
	Попытка
		...
		СчетчикБлоковКонсоли = СтруктураФайлаКонфигурации.КоличествоБлоковКонсоли;
		Для каждого БлокКонфигурации Из СтруктураФайлаКонфигурации.БлокиКонфигурации Цикл
			Если БлокКонфигурации.Удалить Тогда
				Продолжить;
			КонецЕсли;
			ПараметрыФайлаКонфигурации.Поток.ЗаписатьСтроку(БлокКонфигурации.Текст);
			Если БлокКонфигурации.ЭтоБлокКонсоли Тогда
				СчетчикБлоковКонсоли = СчетчикБлоковКонсоли - 1;
			КонецЕсли;
			Если НЕ ЕстьPlanSql
				 И СчетчикБлоковКонсоли = 0 
				...
				ПараметрыФайлаКонфигурации.Поток.ЗаписатьСтроку(БлокPlanSql);
				ЕстьPlanSql = Истина;
			КонецЕсли;
		КонецЦикла;
		ПараметрыФайлаКонфигурации.Поток.ЗаписатьСтроку("</config>" + Символы.ПС);
		ПараметрыФайлаКонфигурации.Поток.Закрыть();
	Исключение
		РезультатВключения.Результат = Ложь;
		РезультатВключения.Причина = НСтр("ru = 'Ошибка записи конфигурационного файла в каталоге'") + " "
	...
	Если ПараметрыФайлаКонфигурации.КаталогКонфигурацииПриложения <> Неопределено 
		...
		Если БлокКонфигурацииТекущегоСеанса = Неопределено Тогда
			...
			БлокКонфигурацииТекущегоСеанса = БлокКонфигурацииСеанса(ФильтрТекущегоСеанса, СтруктураФайлаКонфигурации.ШаблоныТеговКонсоли,
				ПараметрыФайлаКонфигурации.КаталогСЛогФайлами);
		...
		Иначе
			...
			ТекущееСостояниеТЖ = Неопределено;
			ОбойтиУзлыКонфигурацииТехЖурнала(УзелЛог, УзелЛог, ФильтрТекущегоСеанса, Истина,,ТекущееСостояниеТЖ);
...
Функция БлокКонфигурацииСеанса(ФильтрТекущегоСеанса, ШаблоныТегов, КаталогСЛогФайлами)
	...
	Текст = Текст + ШаблоныТегов.Закрывающий + " " + ШаблоныТегов.ОткрывающийОкончание + Символы.ПС;
...
Процедура СоздатьНовуюКонфигурацию(СтруктураФайлаКонфигурации, ПараметрыФайлаКонфигурации, ФильтрТекущегоСеанса)
	...
	ТекстБлокаКонфигурации = "<?xml version=""1.0"" encoding=""UTF-8""?>" + Символы.ПС+ "<config xmlns=""http://v8.1c.ru/v8/tech-log"">"
		+ Символы.ПС + "<dump create=""false"" type=""0"" prntscrn=""false""/>" + Символы.ПС;
	...
	БлокКонфигурации.Вставить("ДатаВремя", Неопределено);
	БлокКонфигурации.Вставить("КаталогТЖ", Неопределено);
	БлокКонфигурации.Вставить("Текст", "</config>");
...
Процедура ОбойтиУзлыКонфигурацииТехЖурнала(КонфигурацияТехЖурнала, ТекущийУзел, ФильтрТекущегоСеанса, ЗаменитьФильтры = Ложь,ФильтрыСобытияТЖ = Неопределено, ТекущееСостояниеТЖ = Неопределено)
	
	Для Каждого Узел Из ТекущийУзел.ДочерниеУзлы Цикл
		
		Если Узел.ИмяУзла = "#document" Или Узел.ИмяУзла = "config" Тогда
			 ОбойтиУзлыКонфигурацииТехЖурнала(КонфигурацияТехЖурнала, Узел, ФильтрТекущегоСеанса, ЗаменитьФильтры,ФильтрыСобытияТЖ);
		КонецЕсли;
		...
		Если Узел.ИмяУзла = "log" Тогда
			...
			Если ФильтрыСобытияТЖ.ТекущееСобытие = Неопределено
				 И ФильтрыСобытияТЖ.ТекущаяБаза = Неопределено
				 И ФильтрыСобытияТЖ.ТекущийПользователь = Неопределено
				 И ФильтрыСобытияТЖ.sessionid = Неопределено
				 И ЗаменитьФильтры Тогда
				МассивСобытий = МассивСобытийКонсолиЗапросов(ИнформационнаяБазаФайловая);
				...
				ТекущееСостояниеТЖ.Вставить("Включен", Истина);
				ТекущееСостояниеТЖ.Вставить("КаталогСЛогФайлами", Узел.ПолучитьАтрибут("location"));
		...
		Если Узел.ИмяУзла = "event" Тогда
			...
			ИначеЕсли НЕ ИнформационнаяБазаФайловая
				 ...
				 И ФильтрыСобытияТЖ.ТекущийПользователь = ФильтрТекущегоСеанса.usr Тогда
					Возврат;
			Иначе
				ФильтрыСобытияТЖ.ТекущееСобытие = Неопределено;
				ФильтрыСобытияТЖ.ТекущаяБаза = Неопределено;
				ФильтрыСобытияТЖ.sessionid = Неопределено;
				ФильтрыСобытияТЖ.ТекущийПользователь = Неопределено;
			КонецЕсли;
...
Процедура ПолучитьПризнакВключенияЖурналаДляТекущегоСеанса(ПараметрыТехнологическогоЖурнала, РезультатВключения) Экспорт
	...
	Если ПараметрыФайлаКонфигурации.КаталогКонфигурацииПриложения <> Неопределено 
		...
		Если БлокКонфигурацииТекущегоСеанса <> Неопределено Тогда
			...
			ТекущееСостояниеТЖ = Неопределено;
			ОбойтиУзлыКонфигурацииТехЖурнала(УзелЛог, УзелЛог, ФильтрТекущегоСеанса, Ложь,,ТекущееСостояниеТЖ);
