...
Процедура СформироватьОтчеты(Команда)
	...
	Если глОтчетОтчет1 Тогда 
		СписокОтчетов.Вставить("ОтчетПродажи");
	КонецЕсли;	
	
	Если глОтчетОтчет1b Тогда 
		СписокОтчетов.Вставить("ОтчетОстатки");
	КонецЕсли;
	
	Если глОтчетПродажиПоРегионам Тогда 
	...
	ПараметрыОжидания.ОповещениеПользователя.НавигационнаяСсылка = "e1cib/app/Обработка.ТК_ФормированиеОтчетовJTI";
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	СхемаКомпоновкиДанных = Обработки.ТК_ФормированиеОтчетовJTI.ПолучитьМакет("ПоляОтбора");
...
Функция НачатьДлительнуюОперацию(СписокОтчетов, Знач ПараметрыОтчетов)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.ТК_ФормированиеОтчетовJTI.СформироватьОтчеты", СписокОтчетов, ПараметрыОтчетов);
