...
Процедура ВыгрузитьДанные() Экспорт 
	...
	ИдентификаторОбработки = "ТК_ВыгрузкаИзменений_УпрДок";		
	КлючОбработки = "ВыгрузкаДокументы_В_УпрДок";
	...
	Исключение 
		...
		КодСостояния = "ОбработкаОбмена.ИнициализацияОбработки - Не удалось подключится к базе ""УпрДок""";						
	...
	ХранилищеДанных	= ОбменДаннымиПереопределяемый.ПравилаКонвертацииОбъектов("ОбменСУпрДок");
	
	Если ХранилищеДанных = Неопределено Тогда
		КодСостояния = "РегистрСведений.ПравилаДляОбменаДанными - Не удалось найти ПравилаКонвертацииОбъектов для ПланыОбмена.ОбменСУпрДок";						
		РегистрыСведений.РезультатыВыполненияОбработок.СообщитьОбОшибке("КодСостояния	"+КодСостояния, СтруктураЗаписиЖурнала);
		РегистрыСведений.РезультатыВыполненияОбработок.ЗафиксироватьРезультатЖурналеВыполненияОбработок(СтруктураЗаписиЖурнала, Ложь);
		Возврат;
	КонецЕсли;
	
	УзелОбмена		= ПланыОбмена.ОбменСУпрДок.НайтиПоКоду("BUU");
	СтатусВыгрузки	= Ложь;
	Ошибки			= "";
	Выгружено		= 0;
