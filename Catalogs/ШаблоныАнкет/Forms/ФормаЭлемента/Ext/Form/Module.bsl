...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Параметры.Ключ.Пустая() ИЛИ Не ПоДанномуШаблонуЕстьАнкеты Тогда 
...
Функция ДобавитьЭлементСправочникаВопросШаблонаАнкеты(СтрокаДерева,Код = Неопределено,СправочникРодитель = Неопределено,ВопросРодитель = Неопределено)
	...
	Если СтрокаДерева.ТипСтроки = "Раздел" Тогда
		...
		СпрОбъект.ВидРасчетаИтогов = СтрокаДерева.ВидРасчетаИтогов;
...
Процедура ОпределитьДоступностьДереваШаблона()
	...
	НедоступностьРедактирования   = ПоДанномуШаблонуЕстьАнкеты;
...
Процедура УстановитьНедоступностьРедактирования()
	...
	Если ПоДанномуШаблонуЕстьАнкеты Тогда
...
Процедура ОткрытьФормуПростыхВопросов(ТекущиеДанные)
	...
	ПростойВопрос.Вставить("ВидРасчетаИтогов", ТекущиеДанные.ВидРасчетаИтогов);
...
&НаКлиенте
Асинх Процедура ЗаполнитьВопросами(Команда)
	Если Объект.Ссылка.Пустая() Тогда
		ОчиститьСообщения();
		Ответ = Ждать ВопросАсинх("Перед заполнением необходимо записать!", РежимДиалогаВопрос.ДаНет, 15, КодВозвратаДиалога.Нет, ,КодВозвратаДиалога.Нет);
		Если Ответ = КодВозвратаДиалога.Да Тогда
			Записан = ЗаписьНаСервереВыполненаУспешно();
	...
	ПараметрыОтбора.Вставить("ВыборГруппИЭлементов ", ИспользованиеГруппИЭлементов.Группы);
	...
	ОбработкаВыбора = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбора", ЭтаФорма,"ПодборВопросов");
	
	ОткрытьФорму("ПланВидовХарактеристик.ВопросыДляАнкетирования.ФормаВыбора",ПараметрыФормы,
...
Процедура ПриЗакрытииФормыВыбора(Значение, ДопПараметры) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВопросыДляАнкетирования.Код КАК Код,
	...
	Пока Выборка.СледующийПоЗначениюПоля("Родитель") Цикл
		Если Выборка.Родитель = Значение Тогда
			Продолжить;
			КонецЕсли;
