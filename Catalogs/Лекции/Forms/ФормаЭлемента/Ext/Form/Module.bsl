...
Процедура ЗагрузитьКонтент(Команда)
	ИмяВСсылке = "lecture_" + СокрЛП(Объект.Код);
	Загрузит(ИмяВСсылке);
КонецПроцедуры
...
Процедура ЗагрузитьКартинку(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		ВызватьИсключение("Необходимо сначала выбрать пункт !!!");
	Иначе
		ИмяВСсылке = "item_" + СокрЛП(Объект.Код) + Формат(ТекущаяДата(),"ДФ=_ггггММддЧЧммсс");
		Загрузит(ИмяВСсылке);
	КонецЕсли;
КонецПроцедуры
...
Процедура Загрузит(ИмяВСсылке)
	...
	Оповещение = Новый ОписаниеОповещения("ЗагрузитьФайлЗавершение", ЭтотОбъект, ИмяВСсылке);
...
Процедура ЗагрузитьФайлЗавершение(РезультатПомещенияФайлов, ИмяВСсылке) Экспорт
	...
	Если СтрНайти(ИмяВСсылке,"lecture") = 0 Тогда
		ТекущиеДанные = Элементы.Содержание.ТекущиеДанные;
		ТекущиеДанные.Картинка = ОбработатьФайлНаСервере(Адрес,РасширениеФайла,ИмяВСсылке);
	Иначе
