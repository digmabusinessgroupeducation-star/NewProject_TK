...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Параметры.Свойство("ХарактеристикаНоменклатуры") Тогда
			Объект.Характеристика = Параметры.ХарактеристикаНоменклатуры;
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ОтветПередЗаписью и ЗначениеЗаполнено(Объект.Номенклатура) Тогда
		...
		ДубльНоменклатурыПоставщика = НайтиДубльНоменклатурыПоставщика(Объект.Ссылка, Объект.Владелец, Объект.Номенклатура, Объект.ХарактеристикаНоменклатуры, Объект.ЕдиницаИзмерения);
...
Процедура НоменклатураПриИзмененииСервер(КэшированныеЗначения)
	...
	СтруктураДействий.Вставить("ПроверитьХарактеристикуПоВладельцу", Объект.ХарактеристикаНоменклатуры);
	СтруктураДействий.Вставить("ПроверитьЗаполнитьУпаковкуПоВладельцу", Объект.ЕдиницаИзмерения);

	СтруктураСтроки = Новый Структура;
	СтруктураСтроки.Вставить("Номенклатура", Объект.Номенклатура);
	СтруктураСтроки.Вставить("ХарактеристикаНоменклатуры", Объект.ХарактеристикаНоменклатуры);
	СтруктураСтроки.Вставить("ХарактеристикиИспользуются", ХарактеристикиИспользуются);
	СтруктураСтроки.Вставить("ЕдиницаИзмерения", Объект.ЕдиницаИзмерения);
	СтруктураСтроки.Вставить("Коэффициент", 1);
...
Функция НайтиДубльНоменклатурыПоставщика(Ссылка, Партнер, Номенклатура, Характеристика, Упаковка)
	...
	Запрос = Новый Запрос("
		...
		|	И НоменклатураПоставщиков.ХарактеристикаНоменклатуры = &Характеристика
		|	И (НоменклатураПоставщиков.ЕдиницаИзмерения = &Упаковка)
