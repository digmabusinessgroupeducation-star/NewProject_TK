...
Функция ПроверкаНабораУсловий(НаборУсловий, ДополнительныеПараметры) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Запрос.УстановитьПараметр("НаборУсловий", НаборУсловий);
	Запрос.УстановитьПараметр("ИдентификаторЧата", ДополнительныеПараметры.ИдентификаторЧата);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТелеграмКонтекстСеансов.Параметр КАК Параметр,
	|	ТелеграмКонтекстСеансов.Значение КАК ЛевоеЗначение
	...
	Для Каждого Найден Из ТаблицаПрограммнаяПроверка Цикл
		Если Найден.ПрограммнаяПроверка = Истина Тогда
			...
			УсловияИстинны = ПрограммнаяПроверка_УсловиеИстинно(Найден.КодПроверки, ДополнительныеПараметры);
			Если УсловияИстинны = Ложь Тогда
				Прервать;
			КонецЕсли;
		КонецЕсли;
	...
	Для Каждого Найден Из ТаблицаУсловия Цикл
		ЛевоеЗначение = ПланыВидовХарактеристик.ТелеграмПараметрыВходящегоСообщения.ЗначениеПараметраВходящегоСообщения(Найден.Параметр, ДополнительныеПараметры.Сообщение);
		УсловияИстинны = ТелеграмСервер.СравнениеИстинно(ЛевоеЗначение, Найден.ВидСравнения, Найден.ПравоеЗначение);
		Если УсловияИстинны = Ложь Тогда
			Прервать;
		КонецЕсли;
