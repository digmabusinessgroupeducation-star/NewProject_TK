...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы          = "СистемныеПроверки";//ГруппаПроверок.Идентификатор;
	Проверка.Наименование                 = НСтр("ru = 'Проверка резервирования заказов покупателя'");
	Проверка.Причины                      = НСтр("ru = 'Резервирование было сформировано после отгрузки или было удалено после корректировки заказа,.'");
	Проверка.Рекомендация                 = НСтр("ru = 'В зависимости от ситуации следует выбрать один из вариантов:
	|Удалить резервирование.
	|Сформировать резервирование.
	|Сформировать корректировку регистра.'");
	Проверка.Идентификатор                = "ПроверитьРезервированиеЗаказовПокупателя";
	Проверка.ОбработчикПроверки           = "ЗапасыСервер.ПроверитьРезервированиеЗаказовПокупателя";
	Проверка.ОбработчикПереходаКИсправлению = "ОбщегоНазначенияТККлиент.СкорректироватьПроблемныеРезервы";
	//Проверка.ДатаНачалаПроверки           = Дата('20140101000000');
	...
	Проверка.Комментарий                  = НСтр("ru = 'Проверка корректности резервирования.'");
