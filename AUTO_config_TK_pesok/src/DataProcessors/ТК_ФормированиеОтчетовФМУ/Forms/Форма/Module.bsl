...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НаДату.Вариант = ВариантСтандартнойДатыНачала.НачалоЭтогоДня;
	ПериодОтчета.Вариант = ВариантСтандартногоПериода.ПрошлаяНеделя;
	ИсточникДанных = "Внутренний";
...
Процедура СформироватьОтчеты(Команда)
	...
	Если глОтчетОстаткиНаСейчас Тогда 
		СписокОтчетов.Вставить("ОтчетОстаткиЕжедневный");
	КонецЕсли;
	
	Если глОтчетПродажиЗаВчера Тогда 
	...
	Если глОтчетПродажиРегион Тогда 
		СписокОтчетов.Вставить("ОтчетПродажиРегион");
	КонецЕсли;
	
	Если глОтчетПродажиРЦ Тогда 
		СписокОтчетов.Вставить("ОтчетПродажиРЦ");
	КонецЕсли;
	
	Если глОтчетПродажиСегмент Тогда 
		СписокОтчетов.Вставить("ОтчетПродажиСегмент");
	КонецЕсли;
	
	Если глОтчетПродажиПроизводитель Тогда 
		СписокОтчетов.Вставить("ОтчетПродажиПроизводитель");
	КонецЕсли;
	
	Если глОтчетПродажиПроизводительРЦ Тогда 
		СписокОтчетов.Вставить("ОтчетПродажиПроизводительРЦ");
	КонецЕсли;
	
	Если глОтчетПродажиРЦТТ Тогда 
		СписокОтчетов.Вставить("ОтчетПродажиРЦТТ");
	КонецЕсли;
	
	Если глОтчетПродажиПроизводительСКЮ Тогда 
		СписокОтчетов.Вставить("ОтчетПродажиПроизводительСКЮ");
	КонецЕсли;
	...
	ПараметрыОтчетов.Вставить("НаДату", КонецДня(НаДату.Дата));
	...
	ПараметрыОжидания.ОповещениеПользователя.НавигационнаяСсылка = "e1cib/app/Обработка.ТК_ФормированиеОтчетовФМУ";
...
Процедура ЗагрузитьНастройкиОтбораПоУмолчанию()
	...
	СхемаКомпоновкиДанных = Обработки.ТК_ФормированиеОтчетовФМУ.ПолучитьМакет("ПоляОтбора");
...
Функция НачатьДлительнуюОперацию(СписокОтчетов, Знач ПараметрыОтчетов)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.ТК_ФормированиеОтчетовФМУ.СформироватьОтчеты", СписокОтчетов, ПараметрыОтчетов);
