...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДокумента = "ТКС" Тогда 				
		...
		ПредставлениеДокумента = НСтр("ru = '""ТКС""'; uk = '""ТКС""'");				
		
	ИначеЕсли ТипДокумента = "Инвентаризация" Тогда		
		
		ПредставлениеДокумента = НСтр("ru = '""Инвентаризация""'; uk = '""Інвентаризація""'");
		
		Если ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(ДополнительныеПараметры, "Результат", Ложь) Тогда 
			ПредставлениеДокумента = ПредставлениеДокумента + " (результат)";
		КонецЕсли;
	...
	ИначеЕсли ТипДокумента = "ВозвратПоставщику" Тогда 
		...
		ПредставлениеДокумента = НСтр("ru = '""Возврат поставщику""'; uk = '""Повернення постачальнику""'");
		
	ИначеЕсли ТипДокумента = "ПеремещениеТоваров" Тогда 
		
		ПредставлениеДокумента = НСтр("ru = '""Перемещение товаров""'; uk = '""Переміщення товарів""'");		
		
	ИначеЕсли ТипДокумента = "ПоступлениеТоваровEDI" Тогда 
		
		ПредставлениеДокумента = НСтр("ru = '""Поступление товаров EDI""'; uk = '""Надходження товарів EDI""'");		
...
Функция НачатьВыполнениеОперации(Операция, ПараметрыОперации = Неопределено)
	...
	Если Операция = "ОбновитьСписокЗагрузки" Тогда 
		...
		Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.ТК_ОбменOpenStore.ЗагрузитьТаблицуДокументов", 
	...
	ИначеЕсли Операция = "ОбновитьСписокВыгрузки" Тогда 
		...
		Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.ТК_ОбменOpenStore.ЗаполнитьТаблицуДокументовДляВыгрузи", 
			ТипДокумента, ПараметрыОперации, АдресТаблицаДокументов);
		
	ИначеЕсли Операция = "СформироватьДокументы" Тогда 
				
		//Возврат ДлительныеОперации.ВыполнитьПроцедуру(, "Обработки.ТК_ОбменOpenStore.ВыполнитьЗагрузкуДокументов", ТипДокумента, СтатусДокументов, Неопределено
		//	, АдресТаблицаДокументов, АдресТаблицаСнятыхЗет , АдресТаблицаТКС);
		
		Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.ТК_ОбменOpenStore.ВыполнитьЗагрузкуДокументов", ТипДокумента, СтатусДокументов, ПараметрыОперации
			, АдресТаблицаДокументов, АдресТаблицаСнятыхЗет , АдресТаблицаТКС);
		
	ИначеЕсли Операция = "ВыгрузитьДокументы" Тогда
				
		//Возврат ДлительныеОперации.ВыполнитьПроцедуру(, "Обработки.ТК_ОбменOpenStore.ВыполнитьВыгрузкуДокументов", ТипДокумента, ПараметрыОперации, АдресТаблицаДокументов);
		
		Возврат ДлительныеОперации.ВыполнитьФункцию(ПараметрыВыполнения, "Обработки.ТК_ОбменOpenStore.ВыполнитьВыгрузкуДокументов", ТипДокумента, ПараметрыОперации, АдресТаблицаДокументов);
