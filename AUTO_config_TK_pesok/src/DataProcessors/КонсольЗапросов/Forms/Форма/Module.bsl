...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не Модифицированность Тогда
		Если НЕ ЗавершениеРаботы Тогда
			Если ПоказыватьПланВыполненияЗапроса Тогда
				Оповестить("ОтключитьПризнакПолученияПланаЗапроса",,ЭтаФорма);
			КонецЕсли;
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Процедура ПередЗакрытиемЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Ответ = КодВозвратаДиалога.Да Тогда
		Оповестить("ОтключитьПризнакПолученияПланаЗапроса");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ОчиститьМеткуЗапроса" Тогда
		МеткаЗапроса = "";
	ИначеЕсли ИмяСобытия = "ОтключитьПризнакПолученияПланаЗапроса"
		 И Источник <> ЭтаФорма Тогда
		ОтключитьПризнакПолученияПланаЗапроса();
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	#Иначе
		...
		Если СтрДлина(ТекстЗапросаВФорме) > 0 Тогда
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	Если ПустаяСтрока(ВерсияПриложения) Тогда
		...
		НаборВерсий =СтрРазделить(СистемнаяИнформация.ВерсияПриложения, ".");
		НаборВерсий.Удалить(3);
		ВерсияПриложения = ТипЧисло.ПривестиЗначение(СтрСоединить(НаборВерсий));
		
	КонецЕсли;
	
	Если ВерсияПриложения > 8311 Тогда
...
Процедура ВыполнитьЗапросСервер(ИндексТекущегоЗапроса, ТабличныйДокументРезультата, ВыводитьВременныеТаблицы, ВыводитьИдентификатор, ТекстЗапроса, ТекстСообщения)
	...
	РезультирующаяСтрока = НСтр("ru = 'Результат запроса (время выполнения = %ВремяВыполнения% с)'");
...
&НаКлиенте
Процедура ОтключитьПризнакПолученияПланаЗапроса()
...
Процедура УстановитьОформлениеНадписиПоказатьПланВыполненияЗапроса()
	...
	Если АнализЗапросаПроведен Тогда
		Если ПланЗапросаАктуален Тогда
			Элементы.ПоказатьПланВыполненияЗапроса.Заголовок = НСтр("ru='Открыть (актуален)'");
		Иначе
