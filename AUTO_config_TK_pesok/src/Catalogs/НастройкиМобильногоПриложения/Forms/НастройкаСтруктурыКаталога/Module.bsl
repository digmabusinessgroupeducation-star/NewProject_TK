...
Процедура ДеревоПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если НЕ Копирование Тогда
		...
		Если ТекСтрока = Неопределено Тогда
			Приемник = ДеревоНастройки;
		Иначе
			Приемник = ДеревоНастройки.НайтиПоИдентификатору(ТекСтрока);
...
Процедура ЭлементДеревоПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	НайденнаяСтрока = ДеревоНастройки.НайтиПоИдентификатору(Строка);
	...
	Если Строка = Неопределено Тогда
		Приемник = ДеревоНастройки;
	Иначе
		Приемник = ДеревоНастройки.НайтиПоИдентификатору(Строка);                                                                                                         
		Если НЕ Приемник.ЭтоГруппа Тогда
			Приемник = Приемник.ПолучитьРодителя();
		КонецЕсли;
	КонецЕсли;	
	...
	ТекущиеЭлементы = ДеревоНастройки.ПолучитьЭлементы();
	
	Для каждого ИДПереносимыйЭлемент из ПараметрыПеретаскивания.Значение Цикл
		
		ПереносимыйЭлемент = ДеревоНастройки.НайтиПоИдентификатору(ИДПереносимыйЭлемент);
...
Процедура ЭлементДеревоПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ИДПриемник = Неопределено Тогда
		Приемник = ДеревоНастройки;
	Иначе
		Приемник = ДеревоНастройки.НайтиПоИдентификатору(ИДПриемник);
		Если НЕ Приемник.ЭтоГруппа Тогда
			Приемник = Приемник.ПолучитьРодителя();
		КонецЕсли;
	КонецЕсли;
	...
	Иначе
		...
		Для Каждого ИДИсточник Из МассивИДИсточник Цикл
			...
			Иначе 
				...
				Источник = ДеревоНастройки.НайтиПоИдентификатору(ИДИсточник);
				...
				Если СтрокаРодитель = Неопределено Тогда
					ДеревоНастройки.ПолучитьЭлементы().Удалить(Источник);
				Иначе
...
Процедура ДетализацияОтбораОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Если ДеревоНастройки.ПолучитьЭлементы().Количество() > 0 Тогда
	
		ПоказатьВопрос(Новый ОписаниеОповещения("ИзменитьДетализациюРезультат", ЭтотОбъект, ВыбранноеЗначение), 
			НСтр("ru='Настройки структуры каталога будут очищены. Продолжить?';uk='Налаштування структури каталогу будуть очищені. Продовжити?'"),
...
Процедура СвернутьВсе(Команда)
	...
	СвернутьДерево(ДеревоНастройки);
...
Процедура РазвернутьВсе(Команда)
	...
	РазвернутьДерево(ДеревоНастройки, 5);
...
Процедура ПоказатьУровень(Команда)
	...
	ТекущийУровень = ПолучитьУровень(ДеревоНастройки.НайтиПоИдентификатору(Элементы.Дерево.ТекущаяСтрока));
...
Процедура ЗаполнитьДеревоИзСохраненныхДанных(ИдентификаторКаталога, АдресНастроекСтруктурыКаталога)
	...
	Иначе	
		ТЗ = РегистрыСведений.МобильноеПриложениеСтруктураКаталога.СоздатьНаборЗаписей();
		ТЗ.Отбор.НастройкаМобильногоПриложения.Установить(НастрокаМП);
		ТЗ.Прочитать();
...
Функция ЗаписатьИЗакрытьСервер()
	...
	СохраненныеДанные.ЗаполнитьЗначения(НастрокаМП, "НастройкаМобильногоПриложения");
...
Процедура ЗаполнитьДеревоИерархии(Группы)
	...
	|	ИерархияНоменклатуры.Номенклатура.Артикул КАК Порядок
	|ИЗ
...
Функция ДеревоКатегорий(КатегорииСписок) Экспорт
	...
	Если Дерево.Строки.Количество() > 1 Тогда
		...
		СтрокаНетКатегории.Представление = НСтр("ru='<Все категории>';uk='<Всі категорії>'");
...
Процедура ПоказатьУровеньДерева(Уровень)
	...
	СвернутьДерево(ДеревоНастройки);
	РазвернутьДерево(ДеревоНастройки, Уровень);
...
Процедура ОтборВыбранныеНаСервере()
	...
	НаборЭлементов = Номенклатура.КомпоновщикНастроек.Настройки.Отбор;

	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(НаборЭлементов,"Выбран",Истина,ВидСравненияКомпоновкиДанных.Равно,,Истина);
...
Процедура ОтборНЕВыбранныеНаСервере()
	...
	ОбновитьВыбранныеТовары(Истина);  
	НаборЭлементов = Номенклатура.КомпоновщикНастроек.Настройки.Отбор;
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(НаборЭлементов,"Выбран",Ложь,ВидСравненияКомпоновкиДанных.Равно,,Истина);
КонецПроцедуры
...
Процедура ОтборВсеНаСервере()
	...
	НаборЭлементов = Номенклатура.КомпоновщикНастроек.Настройки.Отбор;
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(НаборЭлементов,"Выбран",Истина,ВидСравненияКомпоновкиДанных.Равно,,Ложь);
