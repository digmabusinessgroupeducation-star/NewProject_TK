...
Функция ПолучитьНомерВодительскогоУдостоверения(ФизЛицо, Дата = Неопределено) Экспорт 
	...
	Если Рез.Следующий() Тогда 
		НомерВодительскогоУдостоверения = СокрЛП(Рез.Серия) + СокрП(Рез.Номер);
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат НомерВодительскогоУдостоверения;
	
КонецФункции

Функция ПользовательПоКодуФизЛица(ИНН) Экспорт 
	...
	|ГДЕ
	|	Пользователи.ФизическоеЛицо.Код = &Код";
