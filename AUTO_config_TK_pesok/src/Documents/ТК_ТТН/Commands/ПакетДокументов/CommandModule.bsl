...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Для Каждого ТТН ИЗ ПараметрКоманды Цикл
		...
		УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ТК_ТТН", "МаршрутныйЛист,ТТН_Аттика", ТТН, ПараметрыВыполненияКоманды.Источник, Новый Структура());
		
		МассивДляПечати = ПечатьПакетаДокументов(ПараметрКоманды);
		
		Для Каждого строка Из МассивДляПечати Цикл
			УправлениеПечатьюКлиент.ВыполнитьКомандуПечати(строка.ИмяМенеджерПечати, строка.ИменаМакетов, строка.Доки, ПараметрыВыполненияКоманды.Источник, Новый Структура("Порядок,СсылкаТТН","ПакетДокументов",ТТН));
		КонецЦикла;
	КонецЦикла;
